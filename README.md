...
The simplest setup is to open this repo in VS Code and use "Reopen in Container" (requires [Docker Desktop](https://docs.docker.com/desktop/) and the [Dev Containers extension](https://code.visualstudio.com/docs/devcontainers/create-dev-container)). See `.devcontainer/README.md` for container and credential details.

**Note on archival branches:** `main` and `human-preface` carry a devcontainer firewall fix (the firewall is re-applied on every container start and fails closed on setup errors). The archival branches (`backup-ralph/*`, `ralph/run-final`) predate this fix: a container built from them loses its firewall after a container restart and can fail open if firewall setup errors out. If you build a container from an archival branch, copy in `.devcontainer/` from `main` first. I develop on macOS; Windows users (untested) should clone into WSL2 rather than the Windows filesystem (for file I/O, line endings, and symlinks like `AGENTS.md`) and run `Install-Module CredentialManager` in PowerShell so `.credentials/get-credentials.py` can read Windows Credential Manager.
